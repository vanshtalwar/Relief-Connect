        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-2xl max-h-[80vh] flex flex-col bg-[color:var(--muted)] rounded-xl border border-[color:var(--border)] shadow-2xl overflow-hidden relative">
            ...
            <div className="flex-1 overflow-y-auto safe-scrollbar p-5 space-y-3">
              {isModalLoading ? (
                <div className="text-center py-12 text-[13px] text-[color:var(--foreground)]/50">Loading requests...</div>
              ) : modalRequests.length === 0 ? (
                <div className="text-center py-12 text-[13px] text-[color:var(--foreground)]/50">No requests found for this filter.</div>
              ) : (
                modalRequests.map((req) => (
                  <div key={req.id} className="p-4 rounded-xl border border-[color:var(--border)] bg-[color:var(--surface)]/50 hover:bg-[color:var(--surface-strong)] transition-colors">
                    ...
                    <h4 className="text-[14px] font-medium text-[color:var(--foreground)]">{req.title}</h4>
                    <p className="text-[13px] text-[color:var(--foreground)]/70 line-clamp-2 mt-1.5 leading-relaxed">{req.description}</p>
                    <div className="mt-4 flex items-center justify-between">
                      ...
                      <button onClick={() => router.push(`/requests/${req.id}`)} className="text-[12px] font-medium text-[color:var(--foreground)] hover:text-[#38bdf8] transition-colors">View details →</button>
                    </div>
                  </div>
                ))
              )}
